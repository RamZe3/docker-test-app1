...
import {API_URL_MAIN, USERS_API_URL} from "@/common/API";
import {getHash} from "@/common/GuidLogic";
...
export const globalModule = {
    ...
    actions:{
        ...
        checkAuth: async (context) => {
            ...
            const response = await axios.get(API_URL_MAIN +'/login?login=' + login
                + "&password=" + password)
                //.then(() => { store.commit("setLoading", false)});
            if (response.data !== ''){
                console.log(response)
                console.log(login + "олжен быть")
